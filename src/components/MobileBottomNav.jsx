...
export default function MobileBottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-[90%] sm:max-w-md">
      <div className="bg-orange-500 rounded-[2rem] shadow-lg flex items-center justify-around px-2 py-1.5">
        {CUSTOMER_LINKS.map((link) => (
          ...
          >
            ...
            {link.to === '/cart' && cart?.items?.length > 0 && (
              <span className="absolute top-0.5 right-1 w-1.5 h-1.5 bg-[#0033a0] rounded-full"></span>
            )}
            {link.to === '/account/messages' && unreadCount > 0 && isLoggedIn && (
              <span className="absolute top-0.5 right-1 w-2 h-2 bg-[#0033a0] rounded-full border border-orange-500"></span>
            )}
          </NavLink>
        ))}
      </div>
